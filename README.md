...
Untrusted input is the page DOM; it can only ever become one of a fixed set of decisions over a
...
- **Untrusted input:** `fetch<web>` — the page DOM
- **Closed type:** `type Target = Submit | Cancel | Next`, then `type Action = Click(Target) | Fill | Abort` (a closed allow-list of targets)
- **Sink / capability:** `grant operate` → `privileged { operate(d) }`
- **Consequence axes:** Trust

## Run the demo

```sh
examples/apps/browser-extension-agent/demo.sh
```
...
- `browser-extension-agent_safe.wave` — the correct design.
- `browser-extension-agent_unsafe.wave` — the tempting-but-wrong version (the negative example a model must learn to reject).
